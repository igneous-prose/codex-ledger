...
This revision also incorporates additional architecture guidance for Phase 1 and adds backlog items for cache visibility, portable archive handoff, and optional comparison against official workspace analytics.
...
### 11.1 Persistence-aware local discovery
Local discovery must not assume that all Codex work is persisted on disk.

Requirements:
- `doctor` must inspect local Codex configuration when possible and warn if history persistence is disabled or otherwise unavailable
- `sync local` must continue gracefully when local history is partial or absent
- import manifests should record discovery warnings so later reports can explain incomplete historical coverage
- future report metadata should have room for a completeness warning when inputs were known to be incomplete
...
### Doctor requirements
`codex-ledger doctor` must report at least:
- archive home path
- database path
- migration status
- expected local source directories
- whether local history persistence appears enabled, disabled, or unknown
- any discovery warnings that could make imports incomplete
...
- doctor command warns when local history persistence appears disabled or unavailable
...
### 21.5 Cache visibility and cache-efficiency reporting
Reserve room for later reporting that highlights cached vs uncached input tokens, cache-heavy workspaces, and sudden cache misses.

Purpose:
- explain unexpected cost spikes
- show when large instruction payloads or repeated context stopped benefiting from caching
- make later cache diagnostics possible without a schema rewrite

The schema already stores `cached_input_tokens`; use that to keep this feature easy to add later.

### 21.6 Portable archive handoff
Support a later command that exports and re-imports a privacy-safe portable archive bundle so a user can move their local ledger between machines or preserve a stable offline snapshot.

Purpose:
- preserve local history across device changes
- support long-term archival
- avoid pushing users toward cloud sync for a local-first product

This is distinct from a share bundle: the handoff archive is for the user’s own continuity and restoration workflows.

### 21.7 Optional comparison against official workspace analytics
Reserve an extension point for future comparison between the local ledger and official workspace-level analytics or exports when such data is available to the user.

Purpose:
- compare local evidence against official daily totals
- highlight gaps caused by missing local persistence or source coverage
- help advanced users validate local estimates without changing the core local-first model

This is not a v1 deliverable and must remain optional.
...
- raw file copier
- SQLite schema + migrations for immutable raw imports and the canonical event ledger
- local session discovery with `source_kind` recorded from the start
- imported JSON ingestion
- event dedupe
- `doctor` with migration status, source-directory checks, and local history persistence detection
- no daily-rollup tables as canonical storage
- tests
...
If the repository is still brand new, start with **Phase 0** only.
If **Phase 0 is already complete and green**, start with **Phase 1** only.

### For a fresh repo, the first task is:
...
### If Phase 0 is already complete, the next task is Phase 1 only:

1. implement immutable raw import storage under the archive home
2. add or finalize the canonical event-level SQLite schema and migrations
3. record `source_kind`, `provider`, `host`, and provenance on imported rows
4. implement local session discovery and imported JSON ingestion
5. make imports idempotent and content-hash based
6. add `doctor` coverage for migrations, source discovery, and persistence warnings
7. add tests for dedupe, malformed files, workspace signal priority, and rebuild equivalence
8. stop and present a concise Phase 1 status summary before Phase 2

Do **not** jump ahead to pricing, reports, or HTML until Phase 1 is complete.
